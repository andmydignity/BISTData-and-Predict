...
def load_data():
    ...
    for tick in tables:
        ...
        try:
            ...
            # df = df.rename(columns={'foreign_rate': 'ForeignRate'}) # Removed, we use foreign_rate now
            if df.empty:
                continue
            ...
            # Select float columns (all except strings/dates)
...
def train(df):
    logger.info("Preparing Full Training Set (No Split)...")
    ...
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    # df = df.dropna(subset=features) # Allow LightGBM to handle NaNs
    ...
    # Explicit Print
    logger.info(f"Full Train Size: {len(X_train)}")
    ...
    # LightGBM Classifier
    logger.info("Training LightGBM Classifier (FULL DATA / SECTOR RELATIVE)...")
    ...
    try:
        ...
        model.fit(
            ...
            eval_set=[(X_train, y_train)],  # Using train set for logging
            eval_metric="auc",
    ...
    # Calibration Verification (On Training Data)
    # Note: This will be overly optimistic as it matches the training distribution exactly
    y_prob = model.predict_proba(X_train)[:, 1]

    try:
        brier = brier_score_loss(y_train, y_prob)
        print(f"\nCalibration (Train): Brier Score: {brier:.4f}")
    except Exception as e:
        pass
