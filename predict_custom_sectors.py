...
MODEL_PATH = "bist_model_sector_3m.pkl"  # SECTOR MODEL
...
def get_latest_data():
    ...
    for tick in tables:
        ...
        try:
            ...
            try:
                fin_cursor.execute(f"SELECT period FROM {tick}_fin ORDER BY period DESC LIMIT 1")
...
def predict():
    ...
    # 6. Output Custom Sector Lists

    # Define Target Sectors and Counts
    SECTOR_LIMITS = {
        "İMALAT": 25,
        "MALİ KURULUŞLAR": 10,
        "TEKNOLOJİ": 5,
        "ELEKTRİK GAZ VE SU": 3,
    }

    print("\n" + "=" * 120)
    print(f"CUSTOM SECTOR PREDICTIONS (Model: **SECTOR LEADER**)")
    print(
        f"Filters: Vol>30M | SMA<1.3 | RSI<80 | Vol>1.2xAvg | ATR<5% | PD/DD<5 | 0<FD/FAVOK<15 (Excl. Financials)"
    )
    print(f"Date: {df['Date'].max()}")
    print("=" * 120)

    # Format for display (Ensure columns match what is available)
    display_cols = [
        "Ticker",
        "Sector",
        "Price",
    ...
    # Rename columns for compact display (if not already renamed in previous steps, but let's be safe and do it on the copy)
    ...
    for sector, limit in SECTOR_LIMITS.items():
        print(f"\n>>> SECTOR: {sector} (Top {limit})")
        print("-" * 160)

        # Filter by sector
        sector_df = df_filtered[df_filtered["Sector"] == sector].copy()

        if sector_df.empty:
            print("No stocks found passing filters.")
            continue

        # Sort by Win Probability
        sector_top = sector_df.sort_values("Win_Prob", ascending=False).head(limit)

        # Reset Index for Ranking
        sector_top = sector_top.reset_index(drop=True)
        sector_top.index += 1

        print(
            sector_top[display_cols].to_string(
                formatters={
                    "Win_Prob": "{:.2%}".format,
                    "Price": "{:.2f}".format,
                    "StopLoss": "{:.2f}".format,
