...
MODEL_PATH = "bist_model_full.pkl"
...
def load_data():
    ...
    for tick in tables:
        ...
        try:
            ...
            # Note: foreign_rate is lowercase
            cols = [
...
def train(df):
    ...
    # Time Series Split - REMOVED FOR FULL TRAINING
    # Using ALL data for production model
    ...
    # LightGBM Classifier
    logger.info("Training LightGBM Classifier (FULL DATA)...")
    ...
    try:
        ...
        model.fit(
            X_train, y_train,
            eval_set=[(X_train, y_train)], # Using train set for logging only
            eval_metric='auc',
